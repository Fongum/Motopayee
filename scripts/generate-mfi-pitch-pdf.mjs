...
const sourcePath = resolve('docs/mfi-partner-pitch-pack.md');
const outPath = resolve('output/pdf/motopayee-mfi-partner-pitch-pack.pdf');
...
function header() {
  page.text(M, PAGE_H - 28, 'MotoPayee Finance Partner Program', 8, 'F2', '#5c6978');
...
function cover() {
  ...
  page.text(M + 24, PAGE_H - 126, 'Finance Partner Program', 27, 'F2', '#ffffff');
  page.text(M + 24, PAGE_H - 154, 'Structured vehicle finance applications for selected partner institutions.', 11, 'F1', '#dcecff');
  page.color('#1e6b3a');
  page.rect(M + 24, PAGE_H - 186, 150, 20, true);
  page.text(M + 34, PAGE_H - 181, 'Buea + Douala pilot', 8.5, 'F2', '#ffffff');
  page.color('#f5a623');
  page.rect(M + 184, PAGE_H - 186, 154, 20, true);
  page.text(M + 194, PAGE_H - 181, 'Success commission model', 8.5, 'F2', '#172033');
  page.y = PAGE_H - 248;
}
...
function h2(text) {
  ...
  page.text(M, page.y, text, 13.5, 'F2', '#143a63');
  page.y -= 7;
...
function numbered(text) {
  ...
  const bodyLines = wrap(body, PAGE_W - 2 * M - 24, 10);
  page.text(M + 2, page.y, num, 10, 'F2', '#1e6b3a');
  page.text(M + 24, page.y, bodyLines[0], 10);
  page.y -= LINE;
  for (const line of bodyLines.slice(1)) {
    page.text(M + 24, page.y, line, 10);
    page.y -= LINE;
  }
}
...
function parseMarkdown(md) {
  ...
  for (const raw of lines) {
    ...
    if (line.includes('MotoPayee does not lend money') || line.includes('Financing is subject')) {
      note(line);
      continue;
    }
    para(line);
