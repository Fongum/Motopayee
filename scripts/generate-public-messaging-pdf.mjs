...
const sourcePath = resolve('docs/public-launch-messaging-pack.md');
const outPath = resolve('output/pdf/motopayee-public-launch-messaging-pack.pdf');
...
function header() {
  page.text(M, PAGE_H - 28, 'MotoPayee Public Launch Messaging Pack', 8, 'F2', '#5c6978');
...
function cover() {
  ...
  page.text(M + 24, PAGE_H - 126, 'Public Launch Messaging Pack', 24, 'F2', '#ffffff');
  page.text(M + 24, PAGE_H - 154, 'Consistent public messaging for Cameroon-wide launch campaigns.', 11, 'F1', '#dcecff');
  const badges = ['Cameroon positioning', 'Trust education', 'WhatsApp replies', '14-day calendar'];
  let bx = M + 24;
  for (const badge of badges) {
    const bw = width(badge, 8.5) + 18;
    if (bx + bw > PAGE_W - M - 12) break;
    page.color(badge.includes('14') ? '#f5a623' : '#1e6b3a');
    page.rect(bx, PAGE_H - 186, bw, 20, true);
    page.text(bx + 8, PAGE_H - 181, badge, 8.5, 'F2', badge.includes('14') ? '#172033' : '#ffffff');
    bx += bw + 8;
  }
  page.y = PAGE_H - 248;
}
...
function numbered(text) {
  ...
  const lines = wrap(body, PAGE_W - 2 * M - 24, 9.5);
  ensure(lines.length * LINE + 2);
  page.text(M + 2, page.y, num, 9.5, 'F2', '#1e6b3a');
  page.text(M + 24, page.y, lines[0], 9.5);
  page.y -= LINE;
  for (const line of lines.slice(1)) {
    page.text(M + 24, page.y, line, 9.5);
    page.y -= LINE;
  }
}
...
function table(headers, rows) {
  ...
  const widths = [45, 105, 260, 95];
  const wrappedRows = rows.map((row) => row.map((cell, i) => wrap(cell || ' ', widths[i] - 8, 7.3)));
  const heights = wrappedRows.map((row) => Math.max(20, ...row.map((lines) => lines.length * 9 + 8)));
  ...
  headers.forEach((head, i) => {
    page.text(x + 4, y - 14, head, 7.3, 'F2', '#173b28');
    x += widths[i];
  });
  page.y -= 21;
  rows.forEach((row, ri) => {
...
function parseMarkdown(md) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (line.includes('MotoPayee: The trusted vehicle marketplace') || line.includes('Financing is subject')) {
