...
const sourcePath = resolve('docs/rental-partner-program.md');
const outPath = resolve('output/pdf/motopayee-rental-partner-program.pdf');
...
function header() {
  page.text(M, PAGE_H - 28, 'MotoPayee Rental Partner Program', 8, 'F2', '#5c6978');
...
function cover() {
  ...
  page.text(M + 24, PAGE_H - 126, 'Rental Partner Program', 27, 'F2', '#ffffff');
  page.text(M + 24, PAGE_H - 154, 'Verified vehicle rentals, booking payment, and structured rental terms.', 11, 'F1', '#dcecff');
  const badges = ['Cameroon rental network', 'Verified rentals', 'Booking payment', '10% launch commission'];
  let bx = M + 24;
  for (const badge of badges) {
    ...
    page.text(bx + 8, PAGE_H - 181, badge, 8.5, 'F2', badge.includes('10%') ? '#172033' : '#ffffff');
    bx += bw + 8;
  }
  page.y = PAGE_H - 248;
}
...
function parseMarkdown(md) {
  ...
  for (const raw of lines) {
    ...
    if (line.includes('MotoPayee handles booking payment') || line.includes('The vehicle owner holds the security deposit')) {
      note(line, line.includes('security deposit') ? '#f5a623' : '#1e6b3a');
      continue;
    }
    para(line);
  }
