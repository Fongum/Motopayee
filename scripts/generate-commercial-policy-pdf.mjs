...
const sourcePath = resolve('docs/commercial-policy-pricing.md');
const outPath = resolve('output/pdf/motopayee-commercial-policy-pricing.pdf');
...
function header() {
  page.text(M, PAGE_H - 28, 'MotoPayee Commercial Policy And Pricing', 8, 'F2', '#5c6978');
...
function cover() {
  ...
  page.text(M + 24, PAGE_H - 126, 'Commercial Policy And Pricing', 23, 'F2', '#ffffff');
  page.text(M + 24, PAGE_H - 154, 'Launch pricing rules for supply growth, trust, payments, and partner outcomes.', 11, 'F1', '#dcecff');
  const badges = ['Free listings', '6-month dealer pilot', '10% rental commission', 'Success commission'];
  let bx = M + 24;
  for (const badge of badges) {
    const bw = width(badge, 8.2) + 17;
    if (bx + bw > PAGE_W - M - 12) break;
    page.color(badge.includes('10%') ? '#f5a623' : '#1e6b3a');
    page.rect(bx, PAGE_H - 186, bw, 20, true);
    page.text(bx + 8, PAGE_H - 181, badge, 8.2, 'F2', badge.includes('10%') ? '#172033' : '#ffffff');
    bx += bw + 7;
  }
  page.y = PAGE_H - 248;
}
...
function table(headers, rows) {
  ...
  const widths = headers.length === 3 ? [190, 150, 163] : [210, totalW - 210];
  const wrappedRows = rows.map((row) => row.map((cell, i) => wrap(cell || ' ', widths[i] - 8, 7.8)));
  const heights = wrappedRows.map((row) => Math.max(20, ...row.map((lines) => lines.length * 9.5 + 8)));
  ...
  headers.forEach((head, i) => {
    page.text(x + 4, y - 14, head, 7.8, 'F2', '#173b28');
    x += widths[i];
  });
  page.y -= 21;
  rows.forEach((row, ri) => {
...
function parseMarkdown(md) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (line.includes('should grow supply') || line.includes('should not claim') || line.includes('Pricing should be adjusted')) {
      note(line, line.includes('not claim') ? '#f5a623' : '#1e6b3a');
      continue;
    }
    para(line);
  }
