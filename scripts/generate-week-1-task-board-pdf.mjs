...
const sourcePath = resolve('docs/week-1-task-board.md');
const outPath = resolve('output/pdf/motopayee-week-1-task-board.pdf');
...
function header() {
  page.text(M, PAGE_H - 28, 'MotoPayee Week 1 Task Board', 8, 'F2', '#5c6978');
...
function cover() {
  page.color('#0d1f3c');
  page.rect(M, PAGE_H - 205, PAGE_W - 2 * M, 155, true);
  page.text(M + 22, PAGE_H - 94, 'MotoPayee', 12, 'F2', '#dcecff');
  page.text(M + 22, PAGE_H - 126, 'Week 1 Task Board', 28, 'F2', '#ffffff');
  page.text(M + 22, PAGE_H - 154, 'Action checklist for setting up the launch operating system.', 11, 'F1', '#dcecff');
  const badges = ['WhatsApp setup', 'Seller outreach', 'Rental outreach', 'Dealer calls', 'MFI calls'];
  let bx = M + 22;
  for (const badge of badges) {
    const bw = width(badge, 8.2) + 17;
    if (bx + bw > PAGE_W - M - 12) break;
    page.color(badge.includes('Dealer') ? '#f5a623' : '#1e6b3a');
    page.rect(bx, PAGE_H - 181, bw, 20, true);
    page.text(bx + 8, PAGE_H - 176, badge, 8.2, 'F2', badge.includes('Dealer') ? '#172033' : '#ffffff');
    bx += bw + 7;
  }
  page.y = PAGE_H - 232;
}
