...
const sourcePath = resolve('docs/dealer-pilot-program.md');
const outPath = resolve('output/pdf/motopayee-dealer-pilot-program.pdf');
...
function header() {
  page.text(M, PAGE_H - 28, 'MotoPayee Dealer Pilot Program', 8, 'F2', '#5c6978');
...
function cover() {
  ...
  page.text(M + 24, PAGE_H - 126, 'Dealer Pilot Program', 27, 'F2', '#ffffff');
  page.text(M + 24, PAGE_H - 154, 'Free pilot for selected dealers across Cameroon.', 11, 'F1', '#dcecff');
  const badges = ['Cameroon marketplace', '6-month free pilot', 'Dealer inventory', 'Finance-ready buyers'];
  let bx = M + 24;
  for (const badge of badges) {
    const bw = width(badge, 8.5) + 18;
    if (bx + bw > PAGE_W - M - 12) break;
    page.color(badge.includes('6-month') ? '#f5a623' : '#1e6b3a');
    page.rect(bx, PAGE_H - 186, bw, 20, true);
    page.text(bx + 8, PAGE_H - 181, badge, 8.5, 'F2', badge.includes('6-month') ? '#172033' : '#ffffff');
    bx += bw + 8;
  }
  page.y = PAGE_H - 248;
}
...
function parseMarkdown(md) {
  ...
  for (const raw of lines) {
    ...
    if (line.includes('MotoPayee is a trusted vehicle marketplace for Cameroon') || line.includes('During the pilot, MotoPayee-generated leads')) {
      note(line);
      continue;
    }
    para(line);
