    ...
    >
      ...
      <div className="relative h-44 bg-gray-100 overflow-hidden">
        ...
        <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <CompareButton item={{ id: listing.id, type: 'listing', label: v ? `${v.year} ${v.make} ${v.model}` : 'Véhicule' }} compact />
          <WhatsAppShareButton
            text={`${v ? `${v.year} ${v.make} ${v.model}` : 'Véhicule'} sur MotoPayee ${typeof window !== 'undefined' ? window.location.origin : ''}/listings/${listing.id}`}
            compact
          />
        </div>
      </div>
