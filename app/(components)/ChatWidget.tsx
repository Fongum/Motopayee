  ...
  return (
    <div>
      <button
        ...
        className="w-full text-center border border-brand-navy text-brand-navy font-semibold py-2.5 rounded-xl hover:bg-brand-navy hover:text-white transition text-sm flex items-center justify-center gap-2"
      >
        ...
        <div className="mt-3 border border-gray-200 rounded-xl overflow-hidden bg-white shadow-lg">
              ...
              messages.map((m) => {
                ...
                return (
                  <div key={m.id} className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[80%] rounded-xl px-3 py-2 text-xs ${isMine ? 'bg-brand-navy text-white' : 'bg-gray-100 text-gray-800'}`}>
                      {m.body}
                    </div>
                  </div>
                );
          ...
          <div className="border-t border-gray-100 p-2 flex gap-1.5">
            <input
              ...
              placeholder="Message..."
              className="flex-1 border border-gray-200 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-brand-green"
            />
            <button
              onClick={send}
              disabled={sending || !newMsg.trim()}
              className="bg-brand-navy text-white text-xs font-semibold px-3 py-2 rounded-lg disabled:opacity-50"
            >
              &rarr;
            </button>
          </div>
